   ...
   <q-card>
    ...
    >
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="Nama_Bioskop" :props="props">{{ props.row.Nama_Bioskop }}</q-td>
   ...
   <q-dialog v-model="openedit" v-if="openedit">
      <q-card style="width: 700px; max-width:80vw;">
        ...
        <q-separator/>
          ...
          <q-input label ="Nama_Bioskop" v-model ="activedata.Nama_Bioskop"></q-input>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'Nama_Bioskop', align: 'center', label: 'Nama_Bioskop', field: 'Nama_Bioskop', sortable: true },
  ...
  methods: {
    getData () {
      this.$axios.get('databioskop/getAll')
        .then(res => {
    ...
    edit () {
      this.$axios.put('databioskop/edit/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('databioskop/delete/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
